...
public class ConnectAndJoinRandomLb : MonoBehaviour, IConnectionCallbacks, IMatchmakingCallbacks, ILobbyCallbacks
{
    ...
    private void Start()
    {
        ...
        _lbc.AddCallbackTarget(this); //подписка на callback

        _lbc.ConnectUsingSettings(_serverSettings.AppSettings); //подключаемся к серверам Photon
    }

    private void OnDestroy()
    {
        _lbc.RemoveCallbackTarget(this); //обязательно отписка
    }
    ...
    private void Update()
    {
        ...
        _lbc.Service(); //отправка и получение пакетов
